...
// applied the instant the model loads (see onLoaded), so a restored Step 2
// and its Idle_03 are never dropped regardless of script/load ordering.
...
function onLoaded(gltf) {
  ...
  _stallTimer = null;
    ...
    // first) and otherwise falls back to a restored session step read *now*
    // from window.__toriiRestoredStep (deck ran after / its event was missed),
    // so a refresh into Step 2 lands on Step 2 + Idle_03 rather than reverting.
...
// hidden→fade-in transition is deterministic regardless of how quickly a
// cache-hit onLoaded resolves (it never runs synchronously, but the ordering

/* =========================================================
   inspector.js - clip inspector for chiefmonkey-onboarding.glb
...
loader.load(
  '../assets/chiefmonkey-onboarding.glb',
  (gltf) => {
    model = gltf.scene;
    // Same opaque material patch as torii-quest src/napNpc.js. Without it the
    // GLB's alphaMode:BLEND made the skinned mesh split/tear (v0.2.111 fix
    // upstream). frustumCulled=false so bones leaving initial bbox don't
    // cull the whole mesh mid-animation.
    model.traverse((o) => {
      if (o.isMesh) {
        o.castShadow = false;
...
btnExport.addEventListener('click', async () => {
  ...
  const lines = [];
  lines.push('# Clip audit — chiefmonkey-onboarding.glb');
  lines.push('');

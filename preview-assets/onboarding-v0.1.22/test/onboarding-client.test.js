...
describe('loginWithNip07', () => {
  ...
  it('fails when no signer is present (does not throw)', async () => {
    ...
    expect(res.ok).toBe(false);
    expect(res.reason).toMatch(/browser extension signer found/i);
    expect(res.reason).toMatch(/NIP-07/);
  });
});
...
describe('step-1 UI copy + no CDN regression', () => {
  ...
  it('step-1 signer button says "Sign in with browser extension", never "Wallet"', () => {
    expect(step1).toContain('Sign in with browser extension');
    expect(step1).not.toContain('Sign with Plebeian Signer');
    expect(step1).not.toContain('Wallet');
    expect(step1).not.toContain('wallet');
  });

  it('step-1 makes the browser-extension (NIP-07) signer context clear', () => {
    // The lede names the extension category so the button copy is unambiguous.
    expect(step1).toMatch(/browser extension/);
    expect(step1).toMatch(/Plebeian Signer/);
  });
...
// ── v0.1.21: numbered step-dot states (current prominence + completed fill) ──
describe('step-dot wayfinding states (shared.css)', () => {
  const css = readFileSync(join(PREVIEW_DIR, 'shared.css'), 'utf8');
  const block = (sel) => {
    const i = css.indexOf(sel);
    return i === -1 ? '' : css.slice(i, css.indexOf('}', i) + 1);
  };

  it('defines bronze + brighter-amber tokens', () => {
    expect(css).toMatch(/--bronze:\s*hsl\(/);
    expect(css).toMatch(/--amber-bright:\s*hsl\(/);
  });

  it('completed step is bronze-filled with dark numerals', () => {
    const done = block('.step-dot.done .step-dot-num');
    expect(done).toMatch(/background:\s*var\(--bronze\)/);
    expect(done).toMatch(/color:\s*var\(--bg-void\)/); // dark text on bronze (AA)
  });

  it('current step has a brighter, thicker ring than completed', () => {
    const active = block('.step-dot.active .step-dot-num');
    expect(active).toMatch(/border-color:\s*var\(--amber-bright\)/);
    expect(active).toMatch(/box-shadow:\s*0 0 0 3px var\(--amber-bright\)/);
    // still enlarged for a size cue independent of colour
    expect(active).toMatch(/width:\s*36px/);
  });

  it('provides an explicit keyboard focus ring on the step dots', () => {
    const focus = block('.step-dot:focus-visible');
    expect(focus).toMatch(/outline:\s*2px solid var\(--amber-bright\)/);
  });

  it('introduces no new animation (reduced-motion safe wayfinding)', () => {
    // The state cues are size/fill/box-shadow — no @keyframes added for dots.
    const active = block('.step-dot.active .step-dot-num');
    expect(active).not.toMatch(/animation:/);
  });
});

// ── v0.1.21: README version header is correct (regression: was stale 0.1.19) ──
describe('preview README version header', () => {
  it('README states the current preview version', () => {
    const readme = readFileSync(join(PREVIEW_DIR, 'README.md'), 'utf8');
    const version = readFileSync(join(PREVIEW_DIR, 'VERSION'), 'utf8').trim();
    expect(version).toBe('0.1.22-preview');
    expect(readme).toMatch(/Version:\s*\*\*0\.1\.22-preview\*\*/);
    expect(readme).not.toContain('0.1.19-preview');
  });
});

// ── v0.1.22: fonts are self-hosted; the deck makes zero third-party requests ──
describe('self-hosted fonts (no external hosts)', () => {
  const index = readFileSync(join(PREVIEW_DIR, 'index.html'), 'utf8');
  const inspect = readFileSync(join(PREVIEW_DIR, 'inspect', 'index.html'), 'utf8');
  const fontsCss = readFileSync(join(PREVIEW_DIR, 'fonts.css'), 'utf8');
  const FACES = [
    'cabinet-grotesk-500', 'cabinet-grotesk-700',
    'satoshi-400', 'satoshi-500', 'satoshi-700',
  ];

  it('no page loads anything from the Fontshare CDN any more', () => {
    // Prose mentions in comments are fine; a loadable reference is not.
    const cdn = /(?:src|href)\s*=\s*["'][^"']*fontshare|url\(\s*['"]?[^)'"]*fontshare/i;
    expect(index).not.toMatch(cdn);
    expect(inspect).not.toMatch(cdn);
    expect(fontsCss).not.toMatch(cdn);
  });

  it('loads no stylesheet, script, image or font from an external origin', () => {
    // Matches src="http..." / href="http..." in a loadable position.
    const external = /(?:src|href)\s*=\s*["']https?:\/\//gi;
    expect(index.match(external)).toBeNull();
    expect(inspect.match(external)).toBeNull();
    expect(fontsCss).not.toMatch(/url\(\s*['"]?(?:https?:)?\/\//i);
  });

  it('declares every face locally as woff2 and ships the file', () => {
    for (const face of FACES) {
      expect(fontsCss).toContain(`./fonts/${face}.woff2`);
      const bytes = readFileSync(join(PREVIEW_DIR, 'fonts', `${face}.woff2`));
      // woff2 magic number: 'wOF2'
      expect(bytes.subarray(0, 4).toString('latin1')).toBe('wOF2');
    }
  });

  it('ships woff2 only (no woff/ttf dead weight) and swaps rather than blocks', () => {
    expect(fontsCss).not.toMatch(/format\(\s*['"](?:woff|truetype)['"]\s*\)/);
    const faces = fontsCss.match(/@font-face/g) || [];
    expect(faces).toHaveLength(FACES.length);
    const swaps = fontsCss.match(/font-display:\s*swap/g) || [];
    expect(swaps).toHaveLength(FACES.length);
  });

  it('preloads only the two above-the-fold faces', () => {
    const preloads = index.match(/rel="preload"[^>]*as="font"[^>]*>/g) || [];
...
describe('Step 3/5 markup: progress region + real download (no data-advance no-op)', () => {
  ...
  it('curtain panel ships a real same-origin "Open Continuum now" fallback link', () => {
    ...
    expect(curtain).toMatch(/href="\/continuum\/#\/dashboard"/);
...
describe('resolveContinuumDestination (deterministic, same-origin only)', () => {
  it('defaults to the /continuum/#/dashboard app home (not the marketing page)', () => {
    expect(CONTINUUM_HOME).toBe('/continuum/#/dashboard');
    expect(resolveContinuumDestination({ location: { origin: 'https://chiefmonkey.art' } })).toBe('/continuum/#/dashboard');
  });
...
// ── v0.1.20: claimed Step-3 gating, duplicate-pay guard, app-home landing ──
describe('v0.1.20 · Step-3 claimed-state gating is real, not just attempted', () => {
  const css = readFileSync(join(PREVIEW_DIR, 'shared.css'), 'utf8');
  const src = readFileSync(join(PREVIEW_DIR, 'onboarding-client.js'), 'utf8');

  // ROOT CAUSE: author `display` on .btn-primary/.wallet-choices/.choice-card
  // outranked the UA `[hidden]{display:none}`, so `el.hidden = true` did nothing
  // for the setup CTAs. The canonical override is what actually removes them.
  it('shared.css carries the canonical [hidden] override so el.hidden truly hides', () => {
    expect(css).toMatch(/\[hidden\]\s*\{\s*display:\s*none\s*!important;?\s*\}/);
  });

  it('reaching the verified render stamps the panel data-claimed', () => {
    expect(src).toMatch(/panel\.dataset\.claimed\s*=\s*'1'/);
  });

  it('every setup handler refuses to act once the panel is claimed', () => {
    // quote (invoice request), key-connect (verify), and pay-confirm all gate on
    // the claimed flag — closing the stale-UI / double-click duplicate-pay race.
    const guards = src.match(/if\s*\(\s*(?:payBtn\.disabled\s*\|\|\s*)?panel\.dataset\.claimed\s*\)/g) || [];
    expect(guards.length).toBeGreaterThanOrEqual(3);
  });

  it('the status-only resume path funnels through the single verified render', () => {
    // The bug was a bare summary that left setup controls up. BOTH routstrStatus
    // fallbacks (success + error branch of fetchRecoveryState) now call
    // showClaimed → renderSuccessAdvance, which collapses setup and adds Continue.
    const claimedCalls = src.match(/if \(s && s\.connected\) showClaimed\(s\)/g) || [];
    expect(claimedCalls.length).toBe(2);
    expect(src).toMatch(/renderSuccessAdvance\(statusEl, progressEl, 3, \{ routstr: \{ key_preview \}, balance_sats \}\)/);
    // Step 3 no longer keeps a bare-summary showConnected of its own.
    const step3 = src.slice(src.indexOf('export function initStep3'), src.indexOf('export function initStep5'));
    expect(step3).not.toContain('const showConnected');
  });

  it('the ONE pay-confirm button is permanently disabled after a click', () => {
    expect(src).toMatch(/if \(payBtn\.disabled \|\| panel\.dataset\.claimed\) return;/);
    expect(src).toMatch(/payBtn\.disabled = true;/);
  });
});

describe('v0.1.20 · completion lands on the app dashboard, not marketing', () => {
  const src = readFileSync(join(PREVIEW_DIR, 'onboarding-client.js'), 'utf8');
  const html = readFileSync(join(PREVIEW_DIR, 'index.html'), 'utf8');

  it('CONTINUUM_HOME is the hash-router dashboard route', () => {
    expect(CONTINUUM_HOME).toBe('/continuum/#/dashboard');
    // Never the bare marketing surface.
    expect(CONTINUUM_HOME).not.toBe('/continuum/');
  });

  it('the curtain fallback link points at the dashboard too', () => {
    expect(html).toMatch(/data-curtain-open href="\/continuum\/#\/dashboard"/);
  });

  it('same-origin override validation is preserved (no open redirect)', () => {
    expect(src).toContain('resolveContinuumDestination');
    expect(src).toContain("override.startsWith('/') && !override.startsWith('//')");
  });
});

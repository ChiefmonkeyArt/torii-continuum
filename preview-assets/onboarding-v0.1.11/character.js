...
// v0.1.11: watchdog. If the GLB load ever stalls (preload cache mismatch,
// network hiccup, decoder init failure), retry once with cache-bust after
// 8s so the user isn't stuck staring at an empty stage. Cleared on success.
let _loadStarted = performance.now();
let _stallTimer = setTimeout(() => {
  if (!model) {
    console.warn('[character] GLB stalled >8s, retrying with cache-bust');
    loader.load('./assets/chiefmonkey6.glb?r=' + Date.now(), onLoaded, undefined, onErr);
  }
}, 8000);
...
loader.load('./assets/chiefmonkey6.glb', onLoaded, undefined, onErr
);

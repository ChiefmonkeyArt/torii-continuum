...
/** Whole seconds, floored at zero, as "Ns" for the auto-advance countdown. */
...
export function buildRecoveryKit(kitBody, { revealedKey = null } = {}) {
  ...
  if (b.routstr && b.routstr.connected) {
    ...
    if (b.routstr.balance_sats != null) lines.push(`  Balance:        ${b.routstr.balance_sats} sats`);
  } else {
...
// Shows the redacted key, a short auto-advance countdown, and an immediate
// "Continue now". Advancing is idempotent (fires at most once).
function renderSuccessAdvance(statusEl, region, fromStep, body, seconds = 4) {
  setProgress(region, ONBOARD_PHASES.SUCCESS);
  ...
  renderStatus(
    ...
    `<div class="conn-line"><span class="conn-dot"></span><span>Your Routstr key is claimed, encrypted, and verified${prev ? ` &middot; <code>${prev}</code>` : ''}.</span></div>` +
      `${bal != null ? `<div class="conn-meta">${bal} sats funded.</div>` : ''}` +
      `<div class="ci-actions"><button class="btn-primary" data-continue-now><span>Continue now</span><span class="arrow">&rarr;</span></button> <span class="op-countdown" data-countdown aria-live="polite"></span></div>`,
  );
  const cdEl = statusEl.querySelector('[data-countdown]');
  const btn = statusEl.querySelector('[data-continue-now]');
  ...
  if (cdEl) cdEl.textContent = `Stepping through in ${formatCountdown(remaining)}…`;
  const id = setInterval(() => {
    remaining -= 1;
    if (cdEl) cdEl.textContent = `Stepping through in ${formatCountdown(Math.max(0, remaining))}…`;
    if (remaining <= 0) go();
  }, 1000);
...
export function initStep3(doc = typeof document !== 'undefined' ? document : null) {
  ...
  fetchRecoveryState({}).then((state) => {
    ...
    if (state && state.routstr && state.routstr.connected) {
      renderSuccessAdvance(statusEl, progressEl, 3, { routstr: state.routstr, balance_sats: state.routstr.balance_sats }, 6);
      return;
    }
  ...
  const renderConfirm = (quote) => {
    ...
    payBtn.addEventListener('click', async () => {
      ...
      // impatient re-click can never mint/pay twice.
...
// ─── Step 5 · Recovery kit (v0.1.18) ────────────────────────────────────
//
// Fixes the "download is a no-op that just steps through" bug. The Download
// button now fetches the agent's redacted, secret-free kit and saves it as a
// real file (Blob → object URL). The full Routstr key is NEVER in the default
// kit; it is only included when the operator takes the explicit one-time
// "Reveal full key" action, which hits the confirm-gated, no-store export
// endpoint. Stepping through is a separate, explicit control.
...
export function initStep5(doc = typeof document !== 'undefined' ? document : null) {
  ...
  const revealBtn = panel.querySelector('[data-reveal-key]');
  const continueBtn = panel.querySelector('[data-kit-continue]');

  let kitBody = null;       // redacted agent kit (no secrets)
  let revealedKey = null;   // set ONLY after an explicit one-time reveal

  const renderPreview = () => {
    if (!previewEl) return;
    const b = kitBody || {};
    const rows = [];
    rows.push(['Admin npub', b.admin_npub || '—']);
    rows.push(['Provider', b.provider_host || '—']);
    if (b.routstr && b.routstr.connected) {
      rows.push(['Routstr key', b.routstr.key_preview || '—']);
      if (b.routstr.balance_sats != null) rows.push(['Balance', `${b.routstr.balance_sats} sats`]);
    } else {
      rows.push(['Routstr key', 'not connected']);
    }
    if (b.wallet && b.wallet.connected) {
      const w = b.wallet.wallet || {};
      rows.push(['Wallet', w.wallet_pubkey_prefix ? `${w.wallet_pubkey_prefix}…` : 'connected']);
    }
    previewEl.innerHTML = rows
      .map(([k, v]) => `<div class="kit-row"><span class="kit-key">${k}</span><span class="kit-val">${v}</span></div>`)
      .join('');
  };
  ...
  fetchRecoveryKit({}).then((body) => {
    if (body && body.ok !== false) {
      kitBody = body;
      renderPreview();
      if (revealBtn && body.routstr && body.routstr.connected) revealBtn.hidden = false;
    }
  }).catch(() => {});
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', async () => {
      ...
      const kit = buildRecoveryKit(kitBody, { revealedKey });
      triggerDownload(doc, kit.filename, kit.text);
      setStatus('ok', `<div class="conn-line"><span class="conn-dot"></span><span>Kit downloaded${kit.includes_secret_key ? ' — includes your full key. Treat it like cash.' : ' (no secrets — safe to store).'}</span></div>`);
  ...
  if (revealBtn) {
    revealBtn.addEventListener('click', async () => {
      // Explicit, confirmed, one-time reveal. Never implicit.
      if (typeof window !== 'undefined' && typeof window.confirm === 'function') {
        const ok = window.confirm('Reveal your full Routstr key once and include it in the next download? Treat it like cash.');
        if (!ok) return;
      }
      ...
      if (res.ok && res.key) {
        revealedKey = res.key;
        setStatus('warn', '<div class="conn-line"><span class="conn-dot"></span><span>Full key revealed for this session. Your next download will include it — then it will not be shown again.</span></div>');
      } else {
        revealBtn.disabled = false;
        setStatus('err', `<div class="conn-line"><span class="conn-dot"></span><span>${res.reason || 'could not reveal key'}</span></div>`);
      }
    });
  }
}

...
function renderAddForm() {
  ...
  function submit() {
    errorEl.textContent = '';
    try {
      addMember({ npub: npubInput.value, label: labelInput.value });
      npubInput.value = '';
      labelInput.value = '';
      npubInput.focus();
    } catch (e) {
...
function renderMemberRow(m) {
  const c = m.content;
  const removeBtn = h('button', { class: 'ghost', title: 'Remove from roster', onClick: () => removeMember(c.npub) }, ['Remove']);

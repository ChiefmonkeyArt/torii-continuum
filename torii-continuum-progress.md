...
- New agent tests: `project-sources.test.js` (**35**), `markdown-todo.test.js` (**13**), `project-sources-config.test.js` (**7**, subprocess-driven — validation exits the process), `wallet-health.test.js` (**4**). Extended frontend tests: `src/data/agent.test.js` (**23**, +4 client-fn cases), `src/views/board-structure.test.js` (**14**, +6 imported-card guardrails).
- **Root `vitest run` 853/853** (15 files); agent **`node --test` 150/150**.

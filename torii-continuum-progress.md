...
## v0.2.102-alpha — the deck stops phoning home, and the preview archive stops costing 378 MB (2026-08-02)

One slice, CONT-PREVIEW-FONTS-1. **Not deployed** — code + PR only; the parent hands the operator the deploy block. No app or agent runtime code changed: the functional change is confined to `preview-assets/`, which `vite build` never bundles, plus two pinned constants in `ops/torii-final-cutover.sh` and documentation.

- **The last third party is gone.** The onboarding deck loaded its type faces from `api.fontshare.com`. That is one DNS lookup and one TLS handshake before text can paint, a soft failure mode if the CDN is slow or blocked, and — the part that actually matters here — every visitor's IP, user-agent and referer handed to an outside host on a page whose whole point is sovereign onboarding. Production `src/` has enforced zero external hosts since day one; the preview carried an in-file TODO to catch up. It now has: five faces (Cabinet Grotesk 500/700, Satoshi 400/500/700) ship as `preview-assets/onboarding-v0.1.22/fonts/*.woff2`, declared in a local `fonts.css` that both `index.html` and `inspect/index.html` load, with the two above-the-fold faces preloaded. Self-hosting is permitted under the ITF Free Font Licence.
- **woff2 only, 114 KB.** Anything that can run this deck already needs WebGL2 and ES modules, so woff2 support is a given and the CDN's `woff`/`ttf` fallbacks were dead weight. Shipping one format per face keeps the whole type payload to 114 KB, same-origin, cached with everything else.
- **A quiet finding: the mono face was never loading.** The Fontshare CDN silently ignored the `jetbrains-mono` request — JetBrains Mono is not an ITF family — so `--font-mono` has always resolved to the system monospace stack. Rendering is therefore identical before and after this change, and nothing is self-hosted for the mono role. Adding a real mono face is now a deliberate, separately-weighed decision rather than an accident.
- **`preview-assets/` 378 MB → 13 MB.** Twenty-one frozen version directories plus a `releases/` tarball tree that has been obsolete since v0.1.12 (when the version directory itself became the artifact) were being downloaded by every clone forever. `onboarding-v0.1.0` … `onboarding-v0.1.20` and `releases/` are removed; current and previous previews are retained. Nothing is lost — each deleted version is still reachable from its annotated release tag, and `preview-assets/README.md` now documents the retrieval command and a standing keep-two retention policy.
- **A wrong deploy doc corrected.** `preview-assets/README.md` told the next person to rsync into `/var/www/torii/continuum/onboarding-preview` over SSH as root. That path is not the live one. Verified on the box: `/var/www/torii/onboarding-preview` is a **symlink** into `/var/www/torii/onboarding-preview-releases/<timestamp>-<version>/`, flipped atomically by `ops/torii-final-cutover.sh`, which verifies the annotated tag and the shipped VERSION + CTA *before* it mutates anything and probes the live URL afterwards with automatic rollback. The README now says so and points at the script instead of a hand-rolled rsync.
- **Tests / verification (Node 20.x).** Preview suite **165/165** with five new assertions: no loadable Fontshare reference on any page (prose in comments is allowed, a `src`/`href`/`url()` is not); no external origin in any loadable position anywhere in the deck; every declared face present on disk with a valid `wOF2` magic number; woff2-only with `font-display: swap` on all five faces; exactly two font preloads, both `crossorigin` and `type="font/woff2"`. Full repo **1208/1208**. Cutover shell suite **106/106** after repointing `PREVIEW_DIR_NAME`/`PREVIEW_VERSION` and the CTA fixture at v0.1.22.

## Deployment log — onboarding preview v0.1.21 went live (2026-08-01 17:17Z)

Not a release; recording an operator action so the live-vs-repo picture stays honest.

- **What changed on the box.** `https://chiefmonkey.art/onboarding-preview/` served **0.1.19-preview** for two and a half weeks and now serves **0.1.21-preview** — so the "Sign in with browser extension" copy and the remote-signer path from ONBOARD-UI-1 (v0.2.51-alpha) are finally what a visitor actually sees. Page is 20,595 bytes and returns 200.
- **How.** Deployed from the annotated tag `v0.2.100-alpha` with the audited operator script, dry run first, both clean. The script verified the tag, checked the source VERSION and CTA before touching anything, staged a new release directory, and flipped the symlink atomically: `/var/www/torii/onboarding-preview` → `/var/www/torii/onboarding-preview-releases/onboarding-v0.1.21-20260801T171316Z`. Post-deploy HTTP probe passed.
- **Rollback available for one generation.** Previous target `/var/www/torii/onboarding-preview-releases/20260714T171019Z-v0.1.19` is retained; reverting is a single symlink flip.
- **Still true after the deploy.** The *app* on the box remains behind the repo — the deployed Continuum agent version is unchanged by this, and the preview is a static design-review artifact served alongside it, not part of the app build.

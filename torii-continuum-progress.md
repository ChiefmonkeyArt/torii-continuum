...
- `agent/index.mjs`: removed the explicit `disableRequestLogging: false` (it only restated the default and is deprecated in Fastify v5 / removed in v6). No other app code change — the v4→v5 migration was API-transparent for this daemon (trustProxy allow-list, CORS options, rate-limit `global:false` + per-route `config.rateLimit` + `errorResponseBuilder(req, ctx.ttl)` all unchanged in the v11 plugins).
- `@cashu/cashu-ts` **held at 2.5.3**: it carries **no security advisory** (the registry deprecation is Cashu-spec/maintenance only). A v3-lts (3.7.1) or v4 (4.7.0) migration touches proof/token handling in the wallet money path and eliminates zero advisories, so it is deferred to its own slice rather than folded into a security hotfix.
